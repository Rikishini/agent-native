...
| Action                    | Args                                                                      | Purpose                                                                                                                                                              |
| ------------------------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `request-transcript`      | `--recordingId <id>`                                                      | Preserve a ready native transcript first, otherwise fall back to Builder → Groq → OpenAI cloud transcription. Writes segments + fullText to `recording_transcripts`. |
| `save-browser-transcript` | `--recordingId <id> --fullText "..." [--source web-speech\|macos-native]` | Save the native web/macOS transcript (instant, no key). This is the primary transcript source and queues default-title delegation.                                   |
...
2. **Transcription is the narrow media-pipeline exception.** Transcription runs directly because it takes an audio file, not a prompt. `request-transcript` provider priority: **Builder Gemini 3.1 Flash-Lite** (via a connected Builder.io account or `BUILDER_PRIVATE_KEY`) → **Groq** (`whisper-large-v3-turbo`, fast) → **OpenAI** (`whisper-1`). The browser's Web Speech API and desktop macOS Speech capture native transcripts first via `save-browser-transcript`; cloud providers only fill gaps/refine later. Default titles still go through the agent chat: `regenerate-title` writes a `clips-ai-request-:id` application_state entry and the UI bridge dispatches it with `sendToAgentChat`.
